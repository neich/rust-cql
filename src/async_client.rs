...
impl AsyncClient {
    ...
    pub fn exec_query(&mut self, query_str: &str, con: Consistency) -> Future<RCResult<CqlResponse>, ()> {
    ...
    pub fn exec_prepared(&mut self, ps_id: &str, params: &Vec<CqlValue>, con: Consistency) -> Future<RCResult<CqlResponse>, ()> {
        match self.socket.try_clone() {
            Ok(mut socket) => {
                let mut p = Vec::new();
                p.clone_from(params);
...
/*
    ...
    pub fn exec_batch(&mut self, q_type: BatchType, q_vec: Vec<Query>, con: Consistency) -> Future<RCResult<CqlResponse>, ()>{
